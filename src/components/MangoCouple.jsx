...
const ACTIONS = [
  { type: 'love', label: '사랑', emoji: '💖' },
  { type: 'coffee', label: '커피', emoji: '☕' },
  { type: 'snack', label: '만두', emoji: '🥟' },
  { type: 'fan', label: '선풍기', emoji: '🌀' },
];

// 캐릭터별 전용 대사 — 자유롭게 수정/추가하세요! (배열에서 랜덤으로 하나씩 나옴)
const LINES = {
  mango: {
    love: ['라뷰 💕', '사랑해', '알라뷰', '뽀뽀해줘 💋','낑', '꽁','깡','낑꽁깡','공주 사랑해','내가 더 사랑하는데~?','심장 발싸','공주밖에 없어','평생 붙어있을거야','뀽','뀨','끼잉'],
    coffee: ['와 시원해 ☕', '맛있따', '근데 공주 나 배아포', '커피 먹으니까 약간 속이 이상해','맛있다 커피','수박주스 먹고싶다','델라 저랑 스타벅스 가실래요?','공주가 사주는 커피가 젤 맛있어','한모금만 더 줘','아아 최고얌','커피 마시고 힘내서 걸을게!'],
    fan: ['으앙 시원해🍃', '나 쫄딱 다젖었어', '땀나', '개더웡 💦','손풍기 사주는여자 처음이야','바람 나만 쐬도돼~?','시원행 헤헤','공주도 시원해야대','잎사귀 날아가겟어'],
    snack: ['와 이거 진짜맛있다 🥟', '나 다먹을 수 있어!', '다먹는거 쉽지', '다 먹어도돼?','나 만두 진짜 50개 먹을 수 있어','만두는 비비고가 짱이다진짜','공주도 한입 아~','배 터질때까지 먹을래','육즙 팡','진짜 맛있따','나 배 80%찼어', '진짜 배불러 진짜야'],
    tap: ['껌딱지처럼 붙어도돼~?', '이렇게 해도돼~?', '손 잡아도돼~?', '끼잉','야호','헤헤 얼굴만져줘','공주야 얼굴만져줘','더 만져줘','나 착하지~?','공주 옆이 젤 좋아','헤헤 좋아','볼 비벼도돼~?'],
    levelup: ['레벨 업!🎉', '사랑 진화 💞','공주 사랑 먹고 레벨업!','나 사랑 만렙 갈거야'],
  },
  melang: {
    love: ['라뷰 강아지🐶', '나도 사랑해', '낑', '꽁','깡','낑꽁깡','강아지 사랑해','우리강아지 최고','최고미남 김민겸','겸아 라뷰','내 강아지 평생 해','뀨','김민겸이 최고','평생 사랑해','우리강아지 짱','강아지 사랑해','라뷰 라뷰','겸아 라뷰','김메이슨 사랑햇','알랴뷰','사랑해','사랑해 진~짜 많이'],
    coffee: ['오예 커피☕', '커피 고마웡', '짱!', '라뷰 커피', '물적게 얼음 많이! 땡큐', '야호', '강아지는 커피먹으면 안대','커피는 역시 강아지가 사주는거','카페인 충전 완료','아아가 최고야','한모금 남겨준거야? 감동','아 살거같다','메이슨도 커피마셔요?','메이슨 커피 감사해요','메이슨 이따 커피 한잔 하실래요?'],
    fan: ['살거같다 🍃','윙윙', '와 시원해' , '으앙 개더워', '한번 더 (짝)', '멜팅 멜랑💦','상해 진짜 개덥따','시원행 고마웡','나 날아가겠엉','강아지 짱','나 이거 평생 해줘 사랑해','나 머리 말려줭','메이슨 여기 자리 시원해요','메이슨도 좀 쐬실래요?'],
    snack: ['냠냠! 🥟','냠냠냐냐ㅑ냠', '개마싯다', '얌얌','강아지는 지금 배 얼마나 찼어?', '하나 더 줘','강아지 고마워','강아지는 배 얼마나불러?','강아지가 주는 만두 짱','하이디라오 먹으러갈래?','앙~ 하나만 더','배불렁','겸이가 주는 만두가 세상에서 제일 최고','만두볼 됐다 나','메이슨도 이거 드셔보실래요?','메이슨 이 집 만두 잘하네요','메이슨 점심 뭐 드셨어요?'],
    tap: ['간지렁', '그만 만졋', '만지지맛', '알라뷰우','알라뷰','간지럿','꺄','꺄아','앙대 간지럽다구','한번만 더 만지면 문다','히히 좋아','뀨우','우리 강아지 왜케 이쁘지~?','우리 강아지 왜케 잘생겼지?'],
    levelup: ['강아지 사랑으로 나 레벨업 🎉', '세상에서 제일 사랑해 💞','겸이 사랑 먹고 레벨업','더 사랑해줘 강아지 💗','결혼할래?'],
  },
};
...
export default function MangoCouple() {
  ...
  return (
    <div>
      <h1 className="mb-3 text-center text-lg font-extrabold tracking-[0.3em] text-gray-600">
        SHANGHAI · TRAVEL
      </h1>
      ...
      {/* 돌보기 버튼 — 선택된 아이에게 적용 */}
      <p className="relative mt-2 text-center text-[9px] font-semibold text-gray-400">
        💝 지금 {CHARACTERS[state.sel].name} 돌보는 중
      </p>
